...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Montserrat:wght@600;700&family=Inter:wght@400;600&family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&display=swap"
        rel="stylesheet" />
...
<body
    class="bg-background text-on-background selection:bg-primary-container selection:text-on-primary-container min-h-screen flex flex-col">
    ...
    @if ($setting->whatsapp_phone)
        <a aria-label="WhatsApp"
            ...
            href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $setting->whatsapp_phone) }}" target="_blank">
            ...
            <span
                class="absolute right-full mr-4 bg-slate-gray text-white text-xs py-1 px-3 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-md">
    ...
    <!-- Top Navigation Bar -->
    <header
        class="fixed top-0 left-0 w-full z-50 bg-surface border-b border-gray transition-all duration-300 h-16 shadow-sm">
        <div class="max-w-container-max mx-auto px-4 md:px-margin-desktop h-full flex justify-between items-center">
            <a href="{{ url('/') }}" class="flex items-center gap-2">
                @if ($setting->logo_path)
                    <img alt="{{ $setting->name }} Logo" class="h-10 w-auto object-contain"
                        src="{{ asset('storage/' . $setting->logo_path) }}" />
                @else
                    <img alt="NJ CONSTRUCTEC Logo" class="h-10 w-auto object-contain"
                        src="{{ asset('img/logo.png') }}" />
                @endif
                <span
                    class="font-headline-md text-xl md:text-headline-md font-bold text-slate-gray">{{ $setting->name }}</span>
            </a>
            ...
            <nav class="hidden md:flex items-center gap-8">
                ...
                <a class="font-label-bold text-label-bold {{ Request::is('nosotros*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-on-surface-variant hover:text-primary' }} transition-all"
                    href="{{ url('/nosotros') }}">Quiénes Somos</a>
                <a class="font-label-bold text-label-bold {{ Request::is('proyectos*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-on-surface-variant hover:text-primary' }} transition-all"
                    href="{{ url('/proyectos') }}">Proyectos</a>
                <a class="font-label-bold text-label-bold {{ Request::is('blog*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-on-surface-variant hover:text-primary' }} transition-all"
                    href="{{ url('/blog') }}">Blog</a>
                <a class="font-label-bold text-label-bold {{ Request::is('contacto*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-on-surface-variant hover:text-primary' }} transition-all"
                    href="{{ url('/contacto') }}">Contacto</a>
            </nav>

            <div class="flex items-center gap-4">
                <a href="{{ url('/contacto') }}"
                    class="hidden lg:inline-block bg-primary text-on-primary px-6 py-2 rounded font-label-bold text-label-bold hover:opacity-90 transition-opacity duration-200">
                    Cotizar Proyecto
                </a>
        ...
        <div id="mobile-drawer"
            class="fixed inset-y-0 right-0 z-50 w-64 bg-surface border-l border-gray shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out md:hidden flex flex-col">
            ...
            <nav class="flex flex-col gap-6 p-6">
                ...
                <a class="font-label-bold text-lg {{ Request::is('nosotros*') ? 'text-primary font-bold' : 'text-on-surface-variant' }}"
                    href="{{ url('/nosotros') }}">Quiénes Somos</a>
                <a class="font-label-bold text-lg {{ Request::is('proyectos*') ? 'text-primary font-bold' : 'text-on-surface-variant' }}"
                    href="{{ url('/proyectos') }}">Proyectos</a>
                <a class="font-label-bold text-lg {{ Request::is('blog*') ? 'text-primary font-bold' : 'text-on-surface-variant' }}"
                    href="{{ url('/blog') }}">Blog</a>
        ...
        <div class="max-w-container-max mx-auto px-4 md:px-margin-desktop grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-gutter">
            <div class="flex flex-col gap-4">
                <div class="flex items-center gap-2">
                    @if ($setting->logo_path)
                        <img alt="{{ $setting->name }} Logo" class="h-8 w-auto invert grayscale object-contain"
                            src="{{ asset('storage/' . $setting->logo_path) }}" />
                    @else
                        <img alt="NJ Logo White" class="h-8 w-auto invert grayscale"
                            src="{{ asset('img/logo.png') }}" />
                    @endif
                    <span class="font-headline-md text-xl text-off-white font-bold">{{ $setting->name }}</span>
                </div>
                <p class="font-body-sm text-body-sm text-outline-variant">Especialistas en instalaciones sanitarias,
                    eléctricas y equipos de bombeo con más de 18 años de experiencia en Perú.</p>

                <!-- Social media -->
                <div class="flex gap-4 mt-3">
                    @if ($setting->facebook_url)
                        <a class="text-outline-variant hover:text-white transition-colors"
                            href="{{ $setting->facebook_url }}" target="_blank" aria-label="Facebook">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M9 8H7v3h2v9h3v-9h3l.5-3H12V6c0-.5.5-1 1-1h2V2h-3C9.5 2 8 3.5 8 5.5V8z" />
                            </svg>
                        </a>
                    @endif
                    @if ($setting->instagram_url)
                        <a class="text-outline-variant hover:text-white transition-colors"
                            href="{{ $setting->instagram_url }}" target="_blank" aria-label="Instagram">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path
            ...
            <div class="flex flex-col gap-4">
                ...
                <ul class="space-y-2">
                    <li><a class="font-body-sm text-outline-variant hover:text-off-white transition-colors"
                            href="{{ url('/nosotros') }}">Quiénes Somos</a></li>
                    <li><a class="font-body-sm text-outline-variant hover:text-off-white transition-colors"
            ...
            <div
                ...
                <div class="flex gap-4">
                    <span class="font-label-bold text-secondary-container">Desarrollado por <a
                            href="https://vesergenperu.com/" target="_blank"
                            class="underline hover:text-white transition-colors font-semibold">GRUPO
                            VESERGENPERU</a></span>
                </div>
            </div>
        </div>
